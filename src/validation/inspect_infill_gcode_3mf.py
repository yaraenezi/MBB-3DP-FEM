"""Inspect Bambu Studio .gcode.3mf archives and quantify sparse infill paths."""
...
def inspect_archive(path: Path) -> tuple[dict[str, object], dict[float, list[dict[str, float]]]]:
    ...
    total_path_length = float(sum(orientation_weights.values()))
    axial_fourth_moment = sum(
        weight * math.cos(math.radians(angle)) ** 4
        for angle, weight in orientation_weights.items()
    ) / total_path_length
    transverse_fourth_moment = sum(
        weight * math.sin(math.radians(angle)) ** 4
        for angle, weight in orientation_weights.items()
    ) / total_path_length
    # A uniform planar orientation distribution has <cos(theta)^4> = 3/8.
    # This ratio is an affine road-network screening correction, not an RVE.
    axial_orientation_factor = axial_fourth_moment / (3.0 / 8.0)
    result: dict[str, object] = {
        ...
        "sparse_path_length_mm": total_path_length,
        "orientation_length_fraction": {
            f"{angle:g}": weight / total_path_length
            for angle, weight in sorted_orientations
        },
        "axial_fourth_orientation_moment": axial_fourth_moment,
        "transverse_fourth_orientation_moment": transverse_fourth_moment,
        "axial_orientation_factor_vs_planar_isotropic": axial_orientation_factor,
...
def plot_layers(
    ...
    figure.suptitle(f"Sparse-infill toolpath verification\n{archive_name}", fontsize=14)
    figure.tight_layout()
...
def main() -> None:
    ...
    for archive_path in args.archives:
        ...
        plot_layers(
            archive_path.name,
            segments,
            args.output / f"{stem}_sparse_layers.png",
        )
    (args.output / "infill_gcode_validation.json").write_text(
